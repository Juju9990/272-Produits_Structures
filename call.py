...
class Call(Produit):
    def __init__(self, nominal: str, T: float, fixing: None, strike: float, sous_jacent: str, qty: int, echeancier_simulation: list[pd.Timestamp], echeancier_fixing: list[list[pd.Timestamp, float]], smile: pd.DataFrame, Taux_Fwd: pd.DataFrame, ZC: pd.DataFrame):
    ...
    def MonteCarlo(self):
        ...
        for i in range (MCarlo):
            ...
            for j in range (self.__echeancier_fixing[-1][1]-1):
                ...
                vol_local = vol_local_a_matu(S[j], self.__echeancier_simulation[j], self.__echeancier_simulation, self.__smile, "call", self.__div, self.get_maturite())
                S.append(np.exp(np.log(S[j]) + (self.__Taux_Fwd[self.__Taux_Fwd['Date'] == self.__echeancier_simulation[j]]['TauxFWD'].values[0] - vol_local/2) * delta_t + vol_local * np.sqrt(delta_t) * X[j]))
